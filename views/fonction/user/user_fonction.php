       ...
       $success=false;
        ...
        if(count($data)>0) {
           ...
           $req=$con->prepare("UPDATE user SET user_nom=:user_nom,user_prenom=:user_prenom,
                               username=:username,user_tel=:user_tel,
       ...
       $success=false;
        if(count($data)>0) {
         ...
         if($data['id'] > 0){
           ...
           $req=$con->prepare("insert into user(user_nom,user_prenom,user_tel,password,username,user_sexe,user_photo,id_pf,pass) 
             values(:user_nom,:user_prenom,:user_tel,:password,:username,:user_sexe,:user_photo,:id_pf,:pass)") ;
...
function delete_user($id){
        ...
        $q=$con->prepare('UPDATE user SET user_statut="inactif" where user_id="'.$id.'" '); 
...
function update_user($id){
        ...
        $q=$con->prepare('SELECT  * from user where user_id="'.$id.'" '); 
