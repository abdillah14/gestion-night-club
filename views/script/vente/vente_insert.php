...
$item_ds = $_POST["item_ds"];
 ...
 for($count = 0; $count<count($item_ds); $count++)
 {
  ...
  // {
     $con=dbConnect();
   $req=$con->prepare("insert into vente(id_s,v_code,id_t,v_statut,v_q,v_px,id_user,nom) 
     ...
     // $p=r_p($item_cd_clean);
     $sp=$item_qr_clean-$item_qt_clean;
     $req=$con->query("UPDATE stock SET pr_q='".$sp."' WHERE pr_id='".$item_pr_clean."' LIMIT 1");
    }     
